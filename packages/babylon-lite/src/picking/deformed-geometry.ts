...
export function hasCpuDeformation(mesh: MeshInternal): boolean {
    ...
    return !!mesh._cpuPositions && ((!!morph?.targets && !!morph.weights) || (!!skeleton?.boneMatrices && !!skeleton.joints && !!skeleton.weights));
}
...
function applyMorphPositions(mesh: MeshInternal, out: Float32Array): void {
    const morph = mesh.morphTargets;
    if (!morph?.targets || !morph.weights) {
        return;
    }
...
function applySkinPositions(mesh: MeshInternal, out: Float32Array): void {
    const skeleton = mesh.skeleton;
    if (!skeleton?.boneMatrices || !skeleton.joints || !skeleton.weights) {
        return;
    }
    ...
    for (let v = 0; v < vertexCount; v++) {
        ...
        const y = source[i + 1]!;
        const z = source[i + 2]!;
        const result = skinVec3(skeleton.boneMatrices, skeleton.joints, skeleton.weights, skeleton.joints1 ?? null, skeleton.weights1 ?? null, v, x, y, z, 1);
        out[i] = result[0];
        out[i + 1] = result[1];
        out[i + 2] = result[2];
    }
...
function applySkinNormals(mesh: MeshInternal, out: Float32Array): void {
    ...
    for (let v = 0; v < vertexCount; v++) {
        ...
        const y = source[i + 1]!;
        const z = source[i + 2]!;
        const result = skinVec3(skeleton.boneMatrices, skeleton.joints, skeleton.weights, skeleton.joints1 ?? null, skeleton.weights1 ?? null, v, x, y, z, 0);
        out[i] = result[0];
        out[i + 1] = result[1];
        out[i + 2] = result[2];
    }
