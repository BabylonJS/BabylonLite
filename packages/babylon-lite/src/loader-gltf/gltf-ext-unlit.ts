...
const ext: GltfFeature = {
    ...
    async applyMaterial(mat) {
        ...
        // the linear texel and `color` tints by `baseColorFactor`.  When
        // there is no texture, the 1×1 fallback already bakes the factor into
        // its byte value, so leave `color` at its default [1,1,1].
        const tint: [number, number, number] | undefined = mat.baseColorImage ? [f[0], f[1], f[2]] : undefined;
        return tint ? { mode: "unlit" as const, color: tint } : { mode: "unlit" as const };
    },
};
export default ext;
