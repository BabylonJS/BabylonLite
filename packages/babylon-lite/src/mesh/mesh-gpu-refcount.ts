 ...
 *  Each of those resource objects (`MeshGPU`, `SkeletonData`, `MorphTargetData`,
 *  `ThinInstanceData`) may therefore be referenced by more than one `Mesh`, and each
 *  carries its own optional `_refCount` field for this purpose (see each interface).
 *
 ...
 *
 *  A resource with `_refCount` left `undefined` is implicitly owned by exactly one mesh
 *  (the common case — no clone was ever made), so creation sites don't need to initialize
 *  it. `retainGpuResource` is called once per EXTRA owner (i.e. once per clone);
 *  `releaseGpuResource` is called once per owner that goes away and reports whether it
 *  was the last one.
 *
...
/** @internal Shape shared by every trackable GPU resource wrapper (`MeshGPU`,
 *  `SkeletonData`, `MorphTargetData`, `ThinInstanceData`) — each declares its own
 *  optional `_refCount` field so no separate side-table is needed. */
interface GpuRefCounted {
    _refCount?: number;
}
...
export function retainGpuResource(resource: GpuRefCounted): void {
...
export function releaseGpuResource(resource: GpuRefCounted): boolean {
