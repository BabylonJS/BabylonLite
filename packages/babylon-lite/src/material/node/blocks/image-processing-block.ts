 ...
 *  currently exposed by Lite sceneU: optional input gamma->linear conversion,
 *  exposure, standard exponential tone mapping, gamma encode, clamp, and
 *  contrast. Color grading/vignette/dither are intentionally absent because
 *  Lite does not expose those scene states yet.
 */
...
function imageProcessingHelper(convertInputToLinearSpace: boolean): string {
    ...
    if (sceneU.contrast < 1.0) {
        rgb = mix(vec3<f32>(0.5), rgb, sceneU.contrast);
    } else {
        rgb = mix(rgb, highContrast, sceneU.contrast - 1.0);
    }
