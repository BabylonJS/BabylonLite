...
import { releaseGpuResource } from "../mesh/mesh-gpu-refcount.js";
...
export function attachVat(engine: EngineContext, mesh: Mesh, baked: VatBakeResult, clip?: string): VatHandle {
    ...
    mesh.vat = vat;
    // `skel` may be a GPU resource SHARED with a clone (see mesh-gpu-refcount.ts) — release this mesh's
