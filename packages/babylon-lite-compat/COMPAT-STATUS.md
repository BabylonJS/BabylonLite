...
### ✅ Working (86 scenes, MAD ≈ 0)

`1`, `2`, `4`, `5`, `6`, `9`, `10`, `11`, `13`, `14`, `15`, `16`, `18`, `19`, `28`,
...
| Heightmap ground PBR UV tiling                              | 22                                                                                                    | `CreateGroundFromHeightMap` is wrapped and scene 4 now passes (MAD ≈ 0.07) after wiring `StandardMaterial.disableLighting` (the emissive spot-marker sphere was being lit). **22** still diverges: its ground uses a `PBRMaterial` with `albedoTexture.uScale/vScale = 6`, but Lite's PBR pipeline has **no material-level UV scale** — the Lite-native port bakes `uvScale:[6,6]` into the heightmap geometry and sets `gammaAlbedo`/`usePhysicalLightFalloff:false` + a synthesized ORM texture. Matching this in compat needs either a Lite-core PBR UV-scale uniform or geometry-baked UVs driven from the material's texture tiling (material→mesh coupling not currently available at `CreateGroundFromHeightMap` time).                                                                                                                                                                                                                                                                                                                                                                                         |
