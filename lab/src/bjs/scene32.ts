...
(async function () {
    ...
    const mgr = new SpriteManager("labels", getLabelAtlasDataUrl(), 8, { width: LABEL_ATLAS_INFO.cellWidthPx, height: LABEL_ATLAS_INFO.cellHeightPx }, scene);
    ...
    scene.onBeforeRenderObservable.add(() => {
        ...
        for (let i = 0; i < labels.length; i++) {
            ...
            labels[i]!.position.x = a.x;
            labels[i]!.position.y = a.y - offsetPxY * worldPerPxY; // screen-down → world-down
            labels[i]!.position.z = a.z;
        }
    });
    ...
    for (let i = 0; i < anchors.length; i++) {
        ...
        const d = Vector3.Distance(a, camPos);
        const worldPerPxY = (2 * d * tan) / canvas.height;
        const wy = a.y - offsetPxY * worldPerPxY;
        const cx = vp[0]! * a.x + vp[4]! * wy + vp[8]! * a.z + vp[12]!;
        const cy = vp[1]! * a.x + vp[5]! * wy + vp[9]! * a.z + vp[13]!;
        const cw = vp[3]! * a.x + vp[7]! * wy + vp[11]! * a.z + vp[15]!;
