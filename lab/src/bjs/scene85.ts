// BJS reference for scene 85 — parses the same NME matrix JSON as Lite.
...
(async function () {
    ...
    const material = NodeMaterial.Parse(SCENE85_NME_JSON, scene);
    material.build(false);

    const plane = MeshBuilder.CreatePlane("nme-matrix-plane", { width: 2.4, height: 1.8 }, scene);
