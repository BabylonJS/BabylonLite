// BJS reference for scene 215 — deterministic CSM directional-shadow scene with
// PBR receivers that mirrors the Lite scene215 (PBR counterpart of #KY0N7T#84):
// a field of 200 green PBR torus-knot casters scattered across a 2000-unit area
// drops shadows onto a large PBR ground receiver, lit by a single
// DirectionalLight with a 4-cascade CascadedShadowGenerator (PCF5). A non-caster
// "Base" knot sits at the origin.
//
...
(async function () {
    ...
    // PBR ground receiver: light-gray dielectric (metallic 0, roughness 0.9).
    const ground = MeshBuilder.CreateGround("ground", { width: SCENE_SIZE, height: SCENE_SIZE }, scene);
    ground.receiveShadows = true;
    const groundMat = new PBRMaterial("groundMat", scene);
    groundMat.albedoColor = new Color3(0.6, 0.6, 0.6);
    groundMat.metallic = 0.0;
    groundMat.roughness = 0.9;
    groundMat.usePhysicalLightFalloff = false;
    ground.material = groundMat;

    // Shared green PBR caster material (metallic 0.1, roughness 0.5).
    const knotMat = new PBRMaterial("knotMat", scene);
    knotMat.albedoColor = new Color3(0.1, 0.7, 0.15);
    knotMat.metallic = 0.1;
    knotMat.roughness = 0.5;
    knotMat.usePhysicalLightFalloff = false;
