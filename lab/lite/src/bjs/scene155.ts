...
(async function () {
    ...
    const positiveSlide = new Animation("weightedPositive", "position.x", FRAME_RATE, Animation.ANIMATIONTYPE_FLOAT, Animation.ANIMATIONLOOPMODE_CYCLE);
    ...
    const negativeSlide = new Animation("weightedNegative", "position.x", FRAME_RATE, Animation.ANIMATIONTYPE_FLOAT, Animation.ANIMATIONLOOPMODE_CYCLE);
    ...
    const positiveGroup = new AnimationGroup("weightedPositive", scene);
    positiveGroup.addTargetedAnimation(positiveSlide, box);
    positiveGroup.weight = POSITIVE_WEIGHT;
    positiveGroup.start(true, 1, 0, END_FRAME);

    const negativeGroup = new AnimationGroup("weightedNegative", scene);
    negativeGroup.addTargetedAnimation(negativeSlide, box);
    negativeGroup.weight = NEGATIVE_WEIGHT;
