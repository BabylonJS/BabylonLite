// Reference scene 94 — Babylon.js SpriteManager billboards with params colours.
//
// Mirrors lab/src/bjs/scene54.ts exactly (same camera, boxes, sprite layout)
// but pre-multiplies every sprite's `color` by the same `fx.params` vec4 the
// Lite billboard custom shader applies. The multiply commutes through the
// straight-alpha blend, so the rendered pixels match Lite's
...
(async function () {
    ...
    addSprite(manager, "front-left", [-1.6, 0.7, -2.15], [1.25, 0.8], 8, 0, [1, 1, 1, 0.95]);
    addSprite(manager, "center-behind", [0, 0.05, 0.15], [1.65, 1.05], 13, 0, [1, 1, 1, 0.9]);
    addSprite(manager, "far-right", [1.65, -0.25, 1.45], [1.35, 0.95], 18, 0, [1, 1, 1, 0.88], true, false);
    addSprite(manager, "low-back", [-0.55, -0.95, 1.05], [0.95, 1.25], 26, 0, [1, 1, 1, 0.82], false, true);
...
function addSprite(
    ...
    sprite.color = new Color4(color[0] * PARAMS[0], color[1] * PARAMS[1], color[2] * PARAMS[2], color[3] * PARAMS[3]);
