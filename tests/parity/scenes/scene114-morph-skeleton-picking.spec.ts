 ...
 * hit selection plus deterministic marker rendering.
 */
...
test("Scene 114 — Morph/Skeleton Picking matches Babylon.js reference", async ({ page }, testInfo) => {
    ...
    const hitState = await page.locator("canvas").evaluate((canvas) => ({
        ...
        morphDetailedFace: (canvas as HTMLCanvasElement).dataset.morphDetailedFace,
        skeletonDetailedFace: (canvas as HTMLCanvasElement).dataset.skeletonDetailedFace,
    }));
    ...
    expect(Number(hitState.morphDetailedFace)).toBeGreaterThanOrEqual(0);
    expect(Number(hitState.skeletonDetailedFace)).toBeGreaterThanOrEqual(0);
