...
test("Scene 41 — Billboard Sprite Handles + 3D Parenting matches BJS reference at seekTime=1.5s", async ({ page }, testInfo) => {
    const browser = page.context().browser()!;
    await captureGolden(browser, { sceneId: 41, seekTime: SEEK_TIME });

    await page.goto(`/scene41.html?seekTime=${SEEK_TIME}`);
