...
const BASE_PROMPT = `Follow the instructions in .github/copilot/skills/update-compat-layer.md to reconcile the @babylonjs/lite-compat layer against the latest Babylon.js and Babylon Lite changes. This run is fundamentally about reacting to change: pick up what changed upstream (Task 1) and close API-parity gaps in the compat surface (Task 3) — implementing what is newly possible and adding GPU-free tests. Lab-scene parity (Task 2) is conditional: only drive a lab oracle scene to pixel parity if a new Lite feature or other Lite change this run makes a previously-skipped scene possible; otherwise do no scene work. Update packages/babylon-lite-compat/COMPAT-STATUS.md (including the synced commit SHA and date). Before finishing, run the compat checks from the skill's 'Test coverage' section and make sure they pass: 'pnpm exec vitest run --project compat', the two compat typechecks, ESLint, and Prettier. Only run 'pnpm build:bundle-scenes' if you added anything to packages/babylon-lite/ core this run (to prove tree-shakeability). Do NOT run 'pnpm test:perf' or the Lite parity suite ('pnpm test:parity'); do NOT change bundle-size ceilings or golden reference screenshots. Do not open a pull request or push — the pipeline does that. If there is nothing to change, make no edits.`;
